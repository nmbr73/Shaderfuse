...
function get_fuse_commit_info(fuse)
  ...
  if not (hash and modified) then set_error("git log does not match expected output '"..git_output.."'"); return nil end
...
function installer_code(fuse)
    ...
    fuse.Thumbnail = read_fuse_thumbnail(fuse) ; if not fuse.Thumbnail then return nil end
    fuse.Commit = get_fuse_commit_info(fuse) ; if not fuse.Commit then return nil end
