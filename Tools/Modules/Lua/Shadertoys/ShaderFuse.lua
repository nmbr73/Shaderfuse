...
-- Fuse.Version = "v1.0.0"            -- optional; but if given then it must be in 'v<MAJOR>.<MINOR>.<PATCH>' format
...
function ShaderFuse.init(filepath)
    ...
    assert(info.Fuse.Version:match("^v([0-9]+)%.([0-9]+)%.([0-9]+)$"), "Fuse.Version must be v<MAJOR>.<MINOR>.<PATCH>") -- TODO: führende Nullen ausschließen
    info.Fuse.Version = info.Fuse.Version .. '-alpha' -- in Dev it's always 'alpha'; via installer it's 'beta'; in atom its 'rc' (or nothing)
