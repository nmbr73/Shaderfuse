...
--  if not self:isValid() then return false end
  ...
  if false then
    ...
    -- License: Favela.fuse
    -- License: DisplacementShader.fuse
    -- License: ApollianWithATwist.fuse
    -- License: EnergyPlant.fuse
    -- License: FractalEngine.fuse
    -- License: FractalGutter.fuse
    -- License: BuoyantBalls.fuse
    -- License: IHeartFourier.fuse
    -- URL: IHeartFourier.fuse
    -- License: Seascape.fuse
    -- License: EggHunt.fuse
    -- License: WalkingCubes.fuse
    -- License: FallingCuteBombs.fuse
    -- License: Spilled.fuse
    -- URL: Spilled.fuse
    -- License: CrossDistance.fuse
    ...
    -- Logo: Spilled.fuse
    -- License: CrossDistance.fuse
    -- Logo: WindingMengerTunnel.fuse

    local author_url = ''
    local author_logo = ''
    local shader_license = ''

    if self.shadertoy_license ~= '' then
      shader_license = '\n        License = "'.. self.shadertoy_license ..'",'
      print("License: ".. self.file_filename)
    end


    if self.dctlfuse_authorurl ~= '' then
      author_url = '\n        AuthorURL = "'.. self.dctlfuse_authorurl ..'",'
      print("URL: ".. self.file_filename)
    end

    if self.dctlfuse_authorlogo ~= '' then
      author_logo = "\n        AuthorLogo = '".. self.dctlfuse_authorlogo .."',"
      print("Logo: ".. self.file_filename)
    end

    local sfi_filename = self.file_basepath..self.file_category..'/'..self.file_fusename..'.sfi'
    if not bmd.fileexists(sfi_filename) then
      local f = io.open(sfi_filename,"wb")
      if f then
        f:write([[
info = {
    ...
    Shadertoy = {
        ...
        ID = "]].. self.shadertoy_id ..[[",]].. shader_license ..[[

    },

    Fuse = {
        Author = "]].. self.dctlfuse_author ..[[",
        -- Version = "",
        -- Date = "]].. self.dctlfuse_versionDate ..[[",]].. author_url .. author_logo ..[[
