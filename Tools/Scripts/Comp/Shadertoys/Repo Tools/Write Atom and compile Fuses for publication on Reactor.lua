...
--   <span style="color:#a0c050; font-size:large; font-style:italic; ">... welcome to the shaderverse</span>
-- </center>
...
'<center><img src="data:image/png;base64,'
    ...
    ..'"><br />'..
    YourPackageName..[[ adds some Fuses that utilize DCTL to implement various Shaders
    as found on <a href="https://www.shadertoy.com/">Shadertoy.com</a>.<br />
    See our repository on <a href="https://github.com/nmbr73/Shadertoys">GitHub</a> for some
    insights and to maybe constribute to this project.<br />
...
for i, fuse in ipairs(fuses.list) do
  ...
  if fuse.file_category ~= currentCategory then
    ...
    OurPackageDescription=OurPackageDescription..
      '  <p>\n'..
      '    '..currentCategory..' Shaders:\n'..
      '    <ul>\n'


  end

  OurPackageDescription=OurPackageDescription..'      <li>'..fuse.file_fusename..'</li>\n'
...
for i, fuse in ipairs(fuses.list) do
  ...
  local prefix = (i<10 and '00'..i or ( i < 100 and '0'..i or ''..i))..'-'

  fuse:read()

  fuse:write(
    TargetFilepath..PackageIdentifier..'/Fuses/',
    prefix..fuse.file_filename
    )
  fuse:purge()

  OurDeployments=OurDeployments..'          "Fuses/'..prefix..fuse.file_filename..'",\n'
end
