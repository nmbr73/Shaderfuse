		...
		ui:VGroup
		{
			...
			ui:HGroup
			{
				...
				ui:TextEdit{
					...
					HTML = "In order to use the new Fuses you will need to restart DaVinci Resolve / Fusion after installation.",
				},
			},
...
-- ----------------------------------------------------
function iterateDirFiles(path, pathmap, additional)
	...
	for k, v in pairs(fi) do
		if (v.Name ~= nil and string.sub(v.Name,0,1) ~= ".") then
			...
			if (v.IsDir == false) then
				...
				iterateDirFiles(path .. v.Name .. "/", pathmap,  additional .. (additional ~="" and "/" or "") .. v.Name)
			end
		end
	end
end
