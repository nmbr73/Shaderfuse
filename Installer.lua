require("string")

local UI_MANAGER = fu.UIManager
local UI_DISPATCHER = bmd.UIDispatcher(UI_MANAGER)
local UI_WINDOW_GEOMETRY = { 100, 100, 800, 400 }

local TARGET_FUSES_SUBDIRECTORY="Shadertoys"
--local TARGET_FUSES_SUBDIRECTORY="DarthShader"
local LIST_OF_FUSES=nil

local MESSAGEBOX_WINDOW = nil



-- ----------------------------------------------------------------------
-- error_print() and ui_print()
--
-- Just because I have no clue how to print to stderr in a way that the
-- Resolve colsole shows it.
-- ----------------------------------------------------------------------

function err_print(message)
	if message~=nil then
		-- print("ERR " .. message)
	end
end

function gui_print(message)
	if message~=nil then
		-- print("GUI " .. message)
	end
end



-- ----------------------------------------------------------------------
-- MESSAGE BOX
-- ----------------------------------------------------------------------


function showMessageBoxWindow(str)

  if MESSAGEBOX_WINDOW==nil then

    MESSAGEBOX_WINDOW = UI_DISPATCHER:AddWindow({
      ID = "ShaderMessageBox",
      WindowTitle = "Shadertoys Installer - Message",
      Geometry = { 150,150,400,200 },
      Composition = comp,
      UI_MANAGER:VGroup {
        ID = "root",
          --UI_MANAGER:TextEdit{ ID="Message", Text = "" }

        UI_MANAGER:Label{
          ID = "Message",
          WordWrap = true,
          Weight = 1,
          ReadOnly = true,
          Flat = true,
          Alignment = { AlignHCenter = false, AlignTop = true, },
          Text = ""
        },



        UI_MANAGER:HGroup {
          Weight = 0,
          UI_MANAGER:HGap(0,1),
          UI_MANAGER:Button{ ID = "Close", Text = "Okay" },
          UI_MANAGER:HGap(0,1),
        },
      },
    })


    function MESSAGEBOX_WINDOW.On.Close.Clicked(ev)
      err_print("msgbox close clicked - ExitLoop")
      UI_DISPATCHER:ExitLoop()
    end

    function MESSAGEBOX_WINDOW.On.ShaderMessageBox.Close(ev)
      err_print("msgbox wondow closed - ExitLoop")
      UI_DISPATCHER:ExitLoop()
    end

  end

  if str~=nil then
    local itm = MESSAGEBOX_WINDOW:GetItems()
    itm.Message.Text = str
  end

  MESSAGEBOX_WINDOW:Show()
	-- UI_DISPATCHER:RunLoop()
	-- msgbox:Hide()
end


-- ----------------------------------------------------------------------
--
-- ----------------------------------------------------------------------

function showInstallSelectWindow()

  local installselectwin = UI_DISPATCHER:AddWindow({

    ID = "ShaderInstallSelect",
    WindowTitle = "Shadertoys Installer - Select Installation mode ...",
    Geometry = UI_WINDOW_GEOMETRY,
    Composition = comp,

    UI_MANAGER:VGroup {
      ID = "root",
      Weight=1,

      logo(),
      UI_MANAGER:VGap(0,1),

      UI_MANAGER:VGroup {
        Weight=0,
        UI_MANAGER:Label{Text="Create Beta Version: ", Alignment = { AlignHCenter = false, AlignVTop = true, }, WordWrap = false, },
        UI_MANAGER:Button{ ID = "LocalCopy",          Text = "Install local copy of Fuses in 'Fuses:'" },
        UI_MANAGER:Button{ ID = "SingleInstallers",   Text = "Create single Fuse-Installers in './'", Enabled=false, },

        UI_MANAGER:Label{Text="Create Release Cadidate: ", Alignment = { AlignHCenter = false, AlignVTop = true, }, WordWrap = false, },
        UI_MANAGER:Button{ ID = "CreateInstaller",    Text = "Create self-contained installer in './'", Enabled=false },
        UI_MANAGER:Button{ ID = "PrepareSuggestion",  Text = "Prepare Reactor suggestion './'", Enabled=false },
      },

      UI_MANAGER:VGap(0,1),

      UI_MANAGER:HGroup {
        UI_MANAGER:HGap(0,1),

        Weight = 0,
        UI_MANAGER:Button{ ID = "Cancel",   Text = "Cancel" },
      }


    },
  })

  function installselectwin.On.LocalCopy.Clicked(ev)
    gui_print("installselectwin.On.LocalCopy.Clicked() called")
    installselectwin:Hide()
    showInstallMainWindow()
    gui_print("installselectwin.On.LocalCopy.Clicked() ... returned from showInstallMainWindow()")
  end

  function installselectwin.On.CreateInstaller.Clicked(ev)
    installselectwin:Hide()
    -- Not implemented yet
    UI_DISPATCHER:ExitLoop()
  end

  function installselectwin.On.PrepareSuggestion.Clicked(ev)
    installselectwin:Hide()
    -- Not implemented yet
    UI_DISPATCHER:ExitLoop()
  end

  function installselectwin.On.Cancel.Clicked(ev)
    gui_print("installselectwin.On.Cancel.Clicked() called")
    UI_DISPATCHER:ExitLoop()
    os.exit()
  end

  function installselectwin.On.ShaderInstallSelect.Close(ev)
    err_print("SelectWin Window closed")
    UI_DISPATCHER:ExitLoop()
    os.exit()
  end

  installselectwin:Show()


end -- function showInstallSelectWindow

-- ----------------------------------------------------------------------
-- MAIN WINDOW
-- ----------------------------------------------------------------------



function showInstallMainWindow()

  gui_print("showInstallMainWindow() called")

  mainwin = UI_DISPATCHER:AddWindow({

    ID = "ShaderInstallMain",
    WindowTitle = "Shadertoys Installer",
    Geometry = UI_WINDOW_GEOMETRY,
    Composition = comp,

    UI_MANAGER:VGroup {
      ID = "root",

      UI_MANAGER:HGroup {

        Weight = 0,

        logo(),

      },

      UI_MANAGER:VGap(5),

      UI_MANAGER:Tree {
        ID = 'Files',
        SortingEnabled=true,
        Events = {  ItemDoubleClicked=true, ItemClicked=true },
        Weight = 2
      },

      UI_MANAGER:VGap(5),

      UI_MANAGER:VGroup {
          Weight = 0,
          UI_MANAGER:CheckBox{ID = 'UseShortcutPrefix', Text = "Use prefix not only for Fuse name, but also for its OpIconString",  Checked=false },
          UI_MANAGER:CheckBox{ID = 'UseShadertoyID',    Text = "Use Shadertoy IDs as Identifiers instead of shortcuts ",            Checked=true  },
          UI_MANAGER:CheckBox{ID = 'UseCategoryPathes', Text = "Use pathes as categrory for 'Add Tool ...' menu",                   Checked=true  },
      },

      UI_MANAGER:VGap(5),

      UI_MANAGER:HGroup{
        Weight = 0,

        UI_MANAGER:Label {
          ID = 'NumberOfFusesLabel',
          Weight = 3.0,
          Alignment = { AlignHCenter = false, AlignVTop = true, },
          WordWrap = false,
        },

        UI_MANAGER:HGap(0,1),
        UI_MANAGER:Button{ ID = "Install", Text = "Install" },
        UI_MANAGER:Button{ ID = "Cancel", Text = "Cancel" },
      },

    },
  })

  -- ----------------------------------------------------------------------

  function mainwin.On.Install.Clicked(ev)
    gui_print("showInstallMainWindow ... On.Install.Clicked() called")
    performFuseInstall()
    gui_print("showInstallMainWindow()On.Install.Clicked() hide main window")
    mainwin:Hide()
    showMessageBoxWindow("The Fuses should now have been installed. Please make sure to restart DaVinci Resolve and/or Fusion for these changes to take effekt.")
    -- UI_DISPATCHER:ExitLoop()
    gui_print("return from mainwin.On.Install.Clicked(ev)")
  end

  function mainwin.On.Cancel.Clicked(ev)
    UI_DISPATCHER:ExitLoop()
    os.exit()
  end

  function mainwin.On.ShaderInstallMain.Close(ev)
    err_print("Main Window closed")
    UI_DISPATCHER:ExitLoop()
    os.exit()
  end


  mainwin_itm = mainwin:GetItems()
  headers = mainwin_itm.Files:NewItem()
  headers.Text[0] = 'Fuse Name'
  mainwin_itm.Files:SetHeaderItem(headers)
  mainwin_itm.Files.ColumnCount = 1


  if directoryExists(fusion:MapPath("Fuses:").."/"..TARGET_FUSES_SUBDIRECTORY.."/.git") then
    showMessageBoxWindow(
        "It seems that you are managing our Shadertoys Fuses using Git already. That's awesome! Forking and cloning us " ..
        "on GitHub obviously is the right and more pro way of doing things. Just use a 'git pull' in the 'Fuses/Shadertoys/' "..
        "directory and you are up to date and ready to go. Looking forward to your pull requests maybe contributing some "..
        "beatutiful shaders?!?")

    -- err_print("found .git RunLoop()")
    -- UI_DISPATCHER:RunLoop()
    -- err_print("back from .git found RunLoop() - quit program")

    -- os.exit()
    gui_print("return from showInstallMainWindow() after msgbox display")
    return
  end


  local root    = getOwnPath()

  LIST_OF_FUSES = fetchFuses(root)


  local l       = LIST_OF_FUSES.head
  local num_wip = 0

  while l do
    if l.Install then
      it = mainwin_itm.Files:NewItem()
      it.Text[0] = l.File
      mainwin_itm.Files:AddTopLevelItem(it)
    else
      num_wip = num_wip +1
    end
    l=l.next
  end

  mainwin_itm.NumberOfFusesLabel.Text= (LIST_OF_FUSES.len - num_wip) .." Fuses to be installed".. (num_wip and " ("..num_wip.." ignored)" or "")

  gui_print("showInstallMainWindow ... Show()")
  mainwin:Show()

  gui_print("return from showInstallMainWindow() at end of function")

end -- function showInstallMainWindow





-- ----------------------------------------------------------------------
-- directoryExists(path)
--
-- Returns true, if path exists and is a directory; false otherwise.
-- ----------------------------------------------------------------------

function directoryExists(path)

	if path==nil or path=="" then
		return false
	end

	path = string.gsub(path,"\\","/")

	local handle = bmd.readdir(path)

	for k, v in pairs(handle) do
		if v.Name~=nil and v.Name==".git" and v.IsDir then
			return true;
		end
	end

	return false;
end



-- ----------------------------------------------------------------------
-- list=fetchFiles(path,[suffix])
--
-- Traverse the directory 'path' and adds all files with the suffix
-- 'suffix' the the 'list'. Files and directories stating with '.' are
-- omitted!
-- ----------------------------------------------------------------------


function fetchFuses(path)

  assert(path)

  local list = { head = nil, tail = nil, len = 0, Root = path }

  if path==nil or path=="" then
    return list
  end

  if string.sub(path,-1) ~= "/" then
    path = path.."/"
  end

  suffix = suffix or ""

  return fetchFuses_rec(path, "", list )
end


function fetchFuses_rec(path, subpath, list)

  local tail 		= head;
	local handle	= bmd.readdir(path .. subpath .. "*")
  local suffix  = ".fuse"

  for k, v in pairs(handle) do
    if (v.Name ~= nil and string.sub(v.Name,0,1) ~= ".") then
      if (v.IsDir == false) then
        if suffix=="" or string.sub(v.Name,-string.len(suffix)) == suffix then

          list.head = {
            next = list.head, File = v.Name, Path = subpath,
            Install = string.sub( v.Name,-(4+string.len(suffix)) ) ~= "_wip"..suffix and true or false
            }

          list.len  = list.len +1

        end
      else
        list = fetchFuses_rec(path, subpath..v.Name.."/", list )
      end
    end
  end

  return list
end



-- ----------------------------------------------------------------------
-- getOwnPath()
--
-- Path this script has been executed from.
-- ----------------------------------------------------------------------

function getOwnPath()
  local path = debug.getinfo(2, "S").source:sub(2)
  path=path:match("(.*/)")
  return path
end



-- ----------------------------------------------------------------------
-- readFuseCode(filepath,config)
--
-- Read the Fuse and replace its configuration with 'config' if the
-- markers were found in the source.
-- ----------------------------------------------------------------------

function readFuseCode(filepath,config)

  local cb  = "FUSE_COFIG::BEGIN"
  local ce  = "FUSE_COFIG::END"

	local f = assert(io.open(filepath, "rb"))
  local t = f:read("*all")
  f:close()

  if config ~= nil then
    pos1 = string.find(t, cb)
    if pos1~=nil then
      pos2 = string.find(t, ce, pos1)
      if pos2~=nil then
        t= string.sub(t,1,pos1+string.len(cb))..config.."-- "..string.sub(t,pos2)
      end
    end
  end

  return t;
end


-- ----------------------------------------------------------------------
-- formatCategory(categoryPath)
--
-- Formats 'categoryPath' to be used as the category in the "Add
-- Tools..." menu, resp the FC_CATEGORY configuration option.
-- ----------------------------------------------------------------------

function formatCategory(cat)

    cat = cat or ""

    cat = cat:gsub("\\","/") -- just in case

    if cat:len()>0 and cat:sub(-1) == "/" then
      cat=cat:sub(1,-2)
    end

    if cat:len()>0 and cat:sub(1,1) == "/" then
      cat=cat:sub(2)
    end

    cat = cat:gsub("/","\\\\")

    return '"'..cat..'"'

end


-- ----------------------------------------------------------------------
-- processFuses(fuses, params)
--
-- With 'fuses' the result of a fetchFiles() call
--
-- Read the file, replace configuration, create target folders and write
-- the file to the destination.
-- ----------------------------------------------------------------------

function processFuses(fuses, params)


  assert(params~=nil)
  assert(params.targetDirectory)
  assert(fuses.Root)

  if params                   == nil    then params = {}                              end
  if params.useCategoryPathes == nil    then params.useCategoryPathes  = true         end
  if params.useShadertoyID    == nil    then params.useShadertoyID     = true         end
  if params.namePrefix        == nil    then params.namePrefix         = "ERR"        end
  if params.useShortcutPrefix == nil    then params.useShortcutPrefix  = true         end
  if params.useNamePrefix     == false  then params.namePrefix         = ""           end
  if params.toolsSubmenu      == nil    then params.toolsSubmenu       = "Shadertoys" end

  if params.targetDirectory   == nil or params.targetDirectory == "" then params.targetDirectory = "Shadertoys" end

  local cfg =  ""
      .. "local FC_SHORTCUT  = " .. (params.useShadertoyID and "false" or "true")   .."\n"
      .. "local FC_DEVEVELOP = false\n"
      .. "local FC_INFOBTN   = 1\n"
      .. "local FC_PREFIX    = \"" .. params.namePrefix  .."\"\n"
      .. "local FC_SCPREFIX  = "..  (params.useShortcutPrefix and "FC_PREFIX" or "\"\"") .. "\n"
      .. "local FC_SUBMENU   = \"".. params.toolsSubmenu.. "\"\n"


  local l = fuses.head


  while l do


    if l.Install then


      local cat = "local FC_CATEGORY  = "..formatCategory(params.useCategoryPathes and l.Path or "").."\n"


      local fuseSourceCode    = readFuseCode(fuses.Root..l.Path..l.File, cat..cfg);

      if fuseSourceCode then

        if l.Path ~= "" then
          bmd.createdir(fusion:MapPath("Fuses:/"..params.targetDirectory.."/"..l.Path))
        end

        -- err_print("write "..params.targetDirectory.."/"..l.Path..l.File)
        local handle = io.open(fusion:MapPath("Fuses:/"..params.targetDirectory.."/"..l.Path..l.File),"wb")
        if handle then
          handle:write(fuseSourceCode)
          handle:close()
        else
          err_print("failed to write "..l.Path..l.File)
        end
      end
    end
    l=l.next
  end
end



-- ----------------------------------------------------------------------
--
-- ----------------------------------------------------------------------

function performFuseInstall()

  assert(LIST_OF_FUSES ~= nil)

  processFuses(LIST_OF_FUSES, {
    useShadertoyID    = mainwin_itm.UseShadertoyID.Checked,
    useCategoryPathes = mainwin_itm.UseCategoryPathes.Checked,
    useShortcutPrefix = mainwin_itm.UseShortcutPrefix.Checked,
    namePrefix        = "BETA",
    toolsSubmenu      = "Shadertoys (beta)",
    targetDirectory   = TARGET_FUSES_SUBDIRECTORY .. '_beta',
    })

end



-- ######################################################################




function logo()
  return UI_MANAGER:Label{
    ID = "",
    WordWrap = false,
    Weight = 0,
    MinimumSize = {274, 63},
    ReadOnly = true,
    Flat = true,
    Alignment = { AlignHCenter = false, AlignTop = true, },
    Text = '<img src="data:image/png;base64,'
    ..''
    ..'">',
  }

end

gui_print("main: call showInstallSelectWindow()")
showInstallSelectWindow()
gui_print("main: enter RunLoop()")
UI_DISPATCHER:RunLoop()
gui_print("main: back from main RunLoop() - quit program")
os.exit()
