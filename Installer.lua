require("string")
local table = require("table")



local ui = fu.UIManager
local disp = bmd.UIDispatcher(ui)
local target = "Shadertoys"

local source_files = nil
local source_count = 0


-- ----------------------------------------------------------------------
-- MAIN WINDOW
-- ----------------------------------------------------------------------

win = disp:AddWindow(
	{
		ID = "ShadertoysWindow",
		WindowTitle = "Shadertoys Installer",
		Geometry = { 100,100,640,480 },
		Composition = comp,

		ui:VGroup
		{
			ID = "root",


			ui:HGroup
			{
				Weight = 0,



				ui:Label{
					WordWrap = false,
					Weight = 0,

					MinimumSize = {274, 63},

					ID = "",
					ReadOnly = true,
					Flat = true,
					Alignment = {
						AlignHCenter = false,
						AlignTop = true,
					},
					Text = '<img src="data:image/png;base64,">'
					,
					-- Events = {AnchorClicked = true},
				},

				ui:Label{
					WordWrap = true,
					Weight = 0.6,
					ID = "",
					ReadOnly = true,
					Flat = true,
					Alignment = {
						AlignHCenter = true,
						AlignTop = false,
					},
					Text = "In order to use the new Fuses\nyou will need to restart DaVinci Resolve / Fusion\nafter installation.",
				},
			},

			ui:Tree
			{	ID = 'Files',
				SortingEnabled=true,
				Events = {  ItemDoubleClicked=true, ItemClicked=true },
				Weight = 2
			},





            ui:VGroup
            {   Weight = 1,
                ui:CheckBox{ID = 'UseNamePrefix', Text = "Use an 'ST-' prefix in Fuse names to avoid name conflicts", Checked=true},
            },

            ui:VGroup
            {   Weight = 1,
                ui:CheckBox{ID = 'UseShadertoyID', Text = "Use Shadertoy IDs as Identifiers instead of shortcuts ", Checked=true},
            },

			ui:VGroup
            {   Weight = 1,
                ui:CheckBox{ID = 'UseCategoryPathes', Text = "Use pathes as categrory for 'Add Tool ...' menu", Checked=true},
            },

			ui:HGroup
			{
				Weight = 0.5,
				ui:TextEdit{ ID="DebugLog", Text = "" },
				Hidden = true,
			},

			ui:HGroup
			{
				Weight = 0,

				ui:Label{
					Weight = 3.0,
					ID = 'NumberOfFusesLabel',
					Alignment = {
						AlignHCenter = false,
						AlignVTop = true,
					},
					WordWrap = false,
				},

				ui:HGap(0,1),
				ui:Button{ ID = "Install", Text = "Install" },
				ui:Button{ ID = "Cancel", Text = "Cancel" },
			},

        },
	})



-- ----------------------------------------------------------------------

itm = win:GetItems()



function dbg_print(msg)
    itm.DebugLog.PlainText = itm.DebugLog.PlainText..msg;
end

function dbg_println(msg)

	dbg_print( (msg==nil and "NIL" or msg)  .."\n");
end

function win.On.Install.Clicked(ev)
	performInstall()
	disp:ExitLoop()
end

function win.On.Cancel.Clicked(ev)
    dbg_println("Cancel Clicked");
	disp:ExitLoop()
end

function win.On.ShadertoysWindow.Close(ev)
	disp:ExitLoop()
end


-- ----------------------------------------------------------------------
-- MESSAGE BOX
-- ----------------------------------------------------------------------


msgbox = disp:AddWindow(
	{
		ID = "ShadertoysMsgBox",
		WindowTitle = "Shadertoys Installer - Message",
		Geometry = { 150,150,400,200 },
		Composition = comp,
		ui:VGroup
		{
			ID = "root",
			ui:HGroup
			{
				Weight = 1,
				ui:TextEdit{ ID="Message", Text = "" }
			},
			ui:HGroup
			{
				Weight = 0,
				ui:Button{ ID = "Close", Text = "Okay" },
			},
		},
	})
msgboxitm = msgbox:GetItems()
function msgbox.On.Close.Clicked(ev)
	disp:ExitLoop()
end
function msgbox.On.ShadertoysMsgBox.Close(ev)
	disp:ExitLoop()
end
function showMessage(str)
	msgboxitm.Message.Text = str
	msgbox:Show()
	disp:RunLoop()
	msgbox:Hide()
end

-- ----------------------------------------------------------------------
-- Little Helpers
-- ----------------------------------------------------------------------

function directoryExists(p)

	p = string.gsub(p,"\\","/")

	local h = bmd.readdir(p)

	for k, v in pairs(h) do
		if v.Name~=nil and v.Name==".git" and v.IsDir then
			return true;
		end
	end

	return false;
end














local source_files = {}


-- ----------------------------------------------------
function iterateDirFiles(path, pathmap, category)
	local fi = bmd.readdir(path .. "*")
	itm = win:GetItems()
	for k, v in pairs(fi) do
		if (v.Name ~= nil and string.sub(v.Name,0,1) ~= ".") then

			if (v.IsDir == false) then

				if (string.sub(v.Name,-5) == ".fuse") then
					it = itm.Files:NewItem()
					it.Text[0] = v.Name
					it.Text[1] = category
					itm.Files:AddTopLevelItem(it)

					source_files = {next = source_files, file =  v.Name, cat=category, root=path }
					source_count = source_count +1;
				end
			else
				--if (category ~= "") then
				--	category = aditional.."/"
				--end

				iterateDirFiles(path .. v.Name .. "/", pathmap,  category .. (category ~="" and "/" or "") .. v.Name)
			end
		end
	end
end




-- ----------------------------------------------------------------------
-- Action!
-- ----------------------------------------------------------------------

if directoryExists(fusion:MapPath("Fuses:").."/"..target.."/.git") then
	showMessage("It seems that you are managing our Shadertoys Fuses using Git. That's awesome! Forking and/or cloning us on GitHub obviously is the right and more pro way of doing things. Just use a 'git pull' in the 'Fuses/Shadertoys/' directory and you are up to date and ready to go.")
	return
end


-- ----------------------------------------------------------------------
-- Prepare Main Window
-- ----------------------------------------------------------------------

itm = win:GetItems()

headers = itm.Files:NewItem()
headers.Text[0] = 'Name'
headers.Text[1] = 'Category'
itm.Files:SetHeaderItem(headers)
itm.Files.ColumnCount = 2
itm.Files.ColumnWidth[0] = 400
itm.Files.ColumnWidth[1] = 150





win:Show()

--[[
itm.Files:Clear()
it = itm.Files:NewItem()
it.Text[0] = "Foo"
it.Text[1] = "bar"
it.Text[2] = "what?"
itm.Files:AddTopLevelItem(it)
]]


function get_fuses()
	local path = debug.getinfo(2, "S").source:sub(2)
	path=path:match("(.*/)")
	--dbgmsg(str)
	iterateDirFiles(path,"","")
end


local configuration_begin 	="FUSE_COFIG::BEGIN"
local configuration_end		="FUSE_COFIG::END"

local configuration=[[
local FC_DEVEVELOP = false
local FC_INFOBTN   = 1
]]

local configuration_category = false

function processFile(source,file,path)



	local f = assert(io.open(source, "rb"))
    local t = f:read("*all")
    f:close()

	pos1 = string.find(t, configuration_begin)

	if pos1~=nil then
		pos2 = string.find(t, configuration_end, pos1)

		if pos2~=nil then
			part1= string.sub(t,1,pos1+string.len(configuration_begin));

			part2= "-- "..string.sub(t,pos2);

			local cat=path

			if not(configuration_category) then
				cat = ""
			end

			if cat ~= "" then
				cat = string.sub(cat,-2,4)
				--cat = string.gsub(p,"/","\\")
			end

			-- CAT NOT TESTED AND NOT WORKING!!!?!
			t=	part1..configuration.."local FC_CATEGORY  = \""..cat.."\"\n"..part2

			-- dbg_println("file='"..file.."' cat='"..cat.."', path='"..path.."'")

		end
	end


	local f = io.open(fusion:MapPath("Fuses:/"..target.."/"..path.. file),"wb")
	f:write(t)
	f:close()

	--dbg_println(t)


end

function performInstall()

	if source_count==0 then
		return
	end

	configuration=configuration.."\n"
		.. "local FC_PREFIX     = " .. (itm.UseNamePrefix.Checked and "true" or "false") .."\n"
		.. "local FC_SHORTCUT   = " .. (itm.UseShadertoyID.Checked and "false" or "true") .."\n"

	configuration_category = itm.UseCategoryPathes.Checked and true or false


	bmd.createdir(fusion:MapPath("Fuses:/"..target.."/"));


	local l = source_files
	while l do
		if l.file~=nil then
			cat= l.cat~='' and l.cat..'/' or ''

			if cat~='' then
				bmd.createdir(fusion:MapPath("Fuses:/"..target.."/"..cat));
--				dbg_println("create='"..cat.."'")
			end
			processFile(l.root .. l.file, l.file, cat)
		end
		l = l.next
	end

end

get_fuses();

itm.NumberOfFusesLabel.Text= source_count.." Fuses to be installed"








disp:RunLoop()
win:Hide()
